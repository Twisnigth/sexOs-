# Dépôt de paquets d'exemple pour `pacman` (sexOs)

Ce dossier est un **dépôt de paquets prêt à servir**. `pacman`, dans sexOs,
...
## 1. Lancer le serveur (sur votre machine HÔTE, pas dans sexOs)
...
> Astuce : depuis sexOs, `ping 192.168.x.y` permet de vérifier que l'hôte est
...
Relancez le serveur HTTP si besoin, puis `pacman -Sy` dans sexOs.

## Pourquoi ce n'est pas « comme Arch Linux »

Il n'existe pas de miroir sexOs public sur internet : le format de paquet
