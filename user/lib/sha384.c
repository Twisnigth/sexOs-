// =============================================================================
//  user/lib/sha384.c -- SHA-384 (cœur SHA-512, IV SHA-384, sortie 48 octets)
// =============================================================================
#include "sha384.h"
...
static void block(sha384_ctx *c, const uint8_t *p) {
    uint64_t w[80];
    for (int i = 0; i < 16; i++) {
...
void sha384_final(sha384_ctx *c, uint8_t out[48]) {
    ...
    uint8_t pad = 0x80; sha384_update(c, &pad, 1);
    uint8_t z = 0; while (c->n != 112) sha384_update(c, &z, 1);
    uint8_t lb[16] = {0};
    for (int i = 0; i < 8; i++) lb[15-i] = (uint8_t)(bits >> (8*i));
    sha384_update(c, lb, 16);
    for (int i = 0; i < 6; i++)                     // 48 octets = 6 mots
